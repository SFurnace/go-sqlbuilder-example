...
//go:generate sqlgen -v 2 -t pkg.Customer -tn t_customer -out dao/customer.go -db EcmAdminDB.DB -map AppID:int64;Uin:int64 -group IndustryGrade:string
...
//go:generate sqlgen -v 2 -t pkg.Node -tn t_node_info -out dao/node.go -db EcmDB.DB -map ZoneID:int;Zone:string -group ISPNum:int;State:string
type Node struct {
...
//go:generate sqlgen -v 2 -t pkg.Device -tn t_device_info -out dao/device.go -db EcmDB.DB -map InstanceID:string -group AppID:int64;Zone:string;Region:string
type Device struct {
