...
//go:generate sqlgen -t Customer -p S -tn t_customer -out customer.go -db EcmAdminDB.DB -pkg pkg -conv AppID:int64;Uin:int64 -group IndustryGrade:string
...
//go:generate sqlgen -t pkg.Node -tn t_node_info -out tmp/node.go -db EcmDB.DB -conv ZoneID:int;Zone:string -group ISPNum:int;State:string
type Node struct {
...
//go:generate sqlgen -t pkg.Device -tv TableDeviceInfo -out tmp/device.go -db EcmDB.DB -conv InstanceID:string -group AppID:int64;Zone:string;Region:string
type Device struct {
